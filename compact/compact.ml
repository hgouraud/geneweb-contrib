let open Dbdisk in
#mod_use "/Users/Henri/GitHub/hgouraud/geneweb/lib/util/mutil.ml"

let fast = ref true in
let step_strings = ref false in
let step_persons = ref false in
let step_families = ref false in

let load_array a = if !fast then a.load_array () in
let clear_array a = a.clear_array () in

let split_sname base i = Mutil.split_sname @@ base.data.strings.get i in
let split_fname base i = Mutil.split_fname @@ base.data.strings.get i in
...
let steps = ref "strings,persons,families" in
let bname = ref "" in
let action = ref None in
let usage = "Usage: " ^ Sys.argv.(0) ^ " [OPTION] ACTIOn base" in
